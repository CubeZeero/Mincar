...
while True:
    ...
    if home_event == '-menu_upload_btn-':
        ...
        if muuid.check_mbl(muuid.get_muuid(), aws_s3_client, aws_info_bn):
            psg.popup_ok('サーバ側からブロックされました', title = window_title, icon = icon_path, modal = True, keep_on_top = True)
                        ...
                        if upload_event == '-upload_coverart-':
                            ...
                            if disctitle_sp == '' or upload_values['-coverartname_input-'] == '' or upload_values['-editkey_input-'] == '' or upload_values['-disctitle_input-'] == '':
                                ...
                                if psg.popup_yes_no('このカバーアートを登録しますか？', title = window_title, icon = icon_path, modal = True, keep_on_top = True) == 'Yes':
                                    ...
                                    aws_s3_client.upload_file(coverart_path, aws_info_bn, registration_data_name, ExtraArgs = {"Metadata":{"edit-key": upload_values['-editkey_input-'], "mgun": muuid.get_mgun(), "muuid": muuid.get_muuid()}})
