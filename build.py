...
version_all = version[0] + version[1] + version[2]
...
with open('build/Readme.txt', encoding = 'UTF-8') as rm_reader:
    rm_text = rm_reader.read().replace(r'{version}', version_all)

with open('build/Readme.txt', 'w', encoding = 'UTF-8') as rm_writer:
    rm_writer.write(rm_text)

if pack_yn == 'y':
    if os.path.isdir('dist/Mincar_' + version_all) : shutil.rmtree('dist/Mincar_' + version_all)
    if os.path.isfile('dist/Mincar_' + version_all + '.zip') : os.remove('dist/Mincar_' + version_all + '.zip')
    shutil.copytree('build', 'dist/Mincar_' + version_all)
    shutil.make_archive('dist/Mincar_' + version_all, 'zip', root_dir = 'dist/Mincar_' + version_all)
